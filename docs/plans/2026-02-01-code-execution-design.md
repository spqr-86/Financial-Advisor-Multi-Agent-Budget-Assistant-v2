...
User: "сравни расходы на еду в январе и феврале"
   ...
   Вызывает execute_analysis_code с:
     ...
     - user_id: идентификатор пользователя
         ↓
   Tool выполняет код через exec() с ограничениями
     - Загружает расходы в pandas DataFrame `df`
     - Запускает код с контролируемыми globals/locals
...
| Безопасность | exec() с ограничениями | Простота, достаточно для whitelist пользователей |
